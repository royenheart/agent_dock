...
- 用户 rename 终端后必须同步名字：**`onDidChangeTerminalState` 不保证在 rename 时触发（实测 renameWithArg/rename 只更新 `Terminal.name`，事件为空）**，所以保留事件同步 `syncTrackedTerminalName` 之外，还必须有 `syncAllTrackedTerminalNames` 的 1s 轮询兜底 + `markClientTerminalsShuttingDown` 里的 deactivate 最终同步，并在 deactivate 等待 `flushClientTerminalPersistence()` 落盘；删掉任何一环 = reload 后名字还原成创建名，属回退（e2e 用 `AgentDockApi.openClientTerminal` 覆盖生产 dist 路径）
...
- **折叠父节点是层级覆盖：同步删除已记录的后代展开态，restore 只能 reveal 祖先链完整的节点**；重新打开父节点时（onExpand）只记录父节点自身，**不得 scheduleRestore 批量重放后代**——否则“A 已折叠、重新打开 A 后 A.a/A.b 又被批量展开”。改回 onExpand 里重排 restore = 层级状态回退
